defmodule GameOfLifeElixir.BoardTest do
  ...
  test "add new cells to alive cells without duplicates" do
    alive_cells = [{1, 1}, {2, 2}]
    new_cells = [{0, 0}, {1, 1}]
    actual_alive_cells = GameOfLifeElixir.Board.add_cells(alive_cells, new_cells) |> Enum.sort
  ...
  test "remove cells which must be killed from alive cells" do
    ...
    kill_cells = [{1, 1}, {2, 2}]
    actual_alive_cells = GameOfLifeElixir.Board.remove_cells(alive_cells, kill_cells)
  ...
  test "alive cell with 2 neighbours lives on to the next generation" do
    ...
    assert GameOfLifeElixir.Board.keep_alive_tick(alive_cells) == expected_alive_cells
  end

  # become_alive_tick
  test "dead cell with three live neighbours becomes a live cell" do
    alive_cells = [{0, 0}, {1, 0}, {2, 0}, {1, 1}]
    born_cells = GameOfLifeElixir.Board.become_alive_tick(alive_cells)
