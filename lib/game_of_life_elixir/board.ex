defmodule GameOfLifeElixir.Board do
  def add_cells(alive_cells, new_cells) do
    alive_cells ++ new_cells |> Enum.uniq
  end
  ...
  def keep_alive_tick(alive_cells) do
    ...
    |> Enum.map(&(Task.Supervisor.async(
                  ...
                  GameOfLifeElixir.Board, :keep_alive_or_nilify, [alive_cells, &1])))
  ...
  def become_alive_tick(alive_cells) do
    GameOfLifeElixir.Cell.dead_neighbours(alive_cells)
    |> Enum.map(&(Task.Supervisor.async(
                  {GameOfLifeElixir.TaskSupervisor, GameOfLifeElixir.NodeManager.random_node},
                  GameOfLifeElixir.Board, :become_alive_or_nilify, [alive_cells, &1])))
    |> Enum.map(&Task.await/1)
    |> remove_nil_cells
  end

  def keep_alive_or_nilify(alive_cells, cell) do
    if GameOfLifeElixir.Cell.keep_alive?(alive_cells, cell), do: cell, else: nil
  end

  def become_alive_or_nilify(alive_cells, dead_cell) do
    if GameOfLifeElixir.Cell.become_alive?(alive_cells, dead_cell), do: dead_cell, else: nil
  end
